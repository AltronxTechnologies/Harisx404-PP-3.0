...
- The smaller Share control retains Copy URL, Copy as Markdown, View as Markdown, Open in ChatGPT, and Open in Claude, with Email added. Opening Claude copies a prompt for pasting; ChatGPT receives a URL-based prompt. On small screens the menu expands inline below the button so it cannot cover the project title.
- **Deployment prerequisite:** apply `migrations/2026_project_case_studies.sql` to the connected Supabase database before saving a project from Admin. The sandbox only has a service-role REST key, not database DDL credentials; a direct read confirmed the new columns are missing (`42703`). Until the migration is applied, existing public detail pages continue to render, but the revised Admin save returns an explicit migration-required error. No owner project records have been mutated to demonstrate the new sections yet.
