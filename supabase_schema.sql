...
VALUES (true, 'The wall remembers', 'Words that echo', 'always.', 'A collection of notes, hellos, and thoughtful messages left by visitors.', 'Community notes', 'Join the wall', 'Continue with GitHub or Google to leave one note on the wall.', 'Leave your mark', 'Share one thoughtful note. It appears immediately and can be managed by the site Admin.', 'The first note is waiting', 'Visitor messages will appear here.', 'Community Wall | Leave Your Mark', 'Read notes from visitors and leave one thoughtful message on Muhammad Haris''s community wall.') ON CONFLICT (id) DO NOTHING;
