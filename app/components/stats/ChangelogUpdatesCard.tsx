...
import { changelogItems } from "@/app/lib/utils";
...
export function ChangelogUpdatesCard({
  ...
  const cardClassName = "group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border-primary bg-bg-primary p-6 transition-all duration-300 hover:border-indigo-400 hover:bg-white dark:bg-[#1A1F2B] dark:hover:bg-[#1A1F2B]";

  // Mobile: Plain div (zero animation overhead)
  if (shouldReduceAnimations) {
    return (
      <Link href="/buildlog" className="block h-full">
            ...
            <div className="absolute left-0 right-0 top-0">
              {recentItems.map((item, index) => (
                ...
                >
                  ...
                  />
                  {/* Entry card */}
                  <div className="z-10 inline-block w-[100px] space-y-px rounded-lg border border-border-primary bg-white dark:bg-[#1A1F2B] px-2 py-1.5 text-xs shadow-sm">
