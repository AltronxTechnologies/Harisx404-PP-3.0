import { Changelog } from "@/app/lib/utils";
import { fetchAndSortChangelogEntrees } from "app/lib/utils";
import { BentoCard } from "./BentoCard";

export async function ChangelogBento() {
  const changelogData = await fetchAndSortChangelogEntrees();
  const changelogItems = changelogData.slice(0, 4);
  ...
  return (
    <BentoCard height="h-[276px]" linkTo="/buildlog" className="group">
...
function EntreeCard({
  ...
  return (
    <div className="z-10 inline-block w-[160px] space-y-px rounded-xl border border-border-primary bg-white dark:bg-[#1A1F2B] px-3 py-2.5 text-xs">
